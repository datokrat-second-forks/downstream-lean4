...
def modifyEntries {α σ} (env : Environment)
  ...
  PersistentEnvExtension.modifyState ext.toPersistentEnvExtension env (fun (entries, s) => (f entries, s))
    asyncMode asyncDecl
...
def setEntries {α σ} (env : Environment)
    ...
    (asyncMode : EnvExtension.AsyncMode := ext.toEnvExtension.asyncMode)
    (asyncDecl : Name := Name.anonymous) : Environment :=
  PersistentEnvExtension.modifyState ext.toPersistentEnvExtension env (fun (_, s) => (entries, s))
    asyncMode asyncDecl
