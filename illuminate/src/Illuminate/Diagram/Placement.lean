...
where
  go (target : Lean.Name) (pfx : Lean.Name) (xform : Matrix) :
...
def beside (v : Vec2) (gap : Float := 0) (a b : Diagram β) : Diagram β :=
  ...
  let offset := dist • v
  let moved := Diagram.transform (Matrix.translate offset.x offset.y) b
...
def hAppendAlign (guide : AlignGuide) (a b : Diagram β) : Diagram β :=
  ...
  | .fraction f =>
    ...
    let dx := envA[Vec2.east] + envB[Vec2.west]
    .compose a (.transform (Matrix.translate dx dy) b)
  | .anchor _ =>
