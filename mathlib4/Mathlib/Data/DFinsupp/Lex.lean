...
instance Colex.decidableLE : DecidableLE (Colex (Π₀ i, α i)) := by
  unsealing_newtype OrderDual => exact Lex.decidableLE (ι := ιᵒᵈ)
...
instance Colex.decidableLT : DecidableLT (Colex (Π₀ i, α i)) := by
  unsealing_newtype OrderDual => exact Lex.decidableLT (ι := ιᵒᵈ)
