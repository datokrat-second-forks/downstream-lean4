...
instance : Ring (euclidean.mixedSpace K) :=
  have : Ring (EuclideanSpace ℝ {w : InfinitePlace K // IsReal w}) := (PiLp.equiv 2 _).ring
  have : Ring (EuclideanSpace ℂ {w : InfinitePlace K // IsComplex w}) := (PiLp.equiv 2 _).ring
...
def toMixed : (euclidean.mixedSpace K) ≃L[ℝ] (mixedSpace K) :=
  (WithLp.linearEquiv _ _ _).trans
    ((PiLp.linearEquiv _ _ _).prodCongr (PiLp.linearEquiv _ _ _)) |>.toContinuousLinearEquiv
